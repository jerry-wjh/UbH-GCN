...
def get_parser():
    ...
    parser.add_argument(
        '--config',
        default='./config/AIDE/bone_root_1.yaml',
    ...
    parser.add_argument(
        ...
        action=DictAction,
        default=dict(),
        help='the arguments of data loader for eval')
    parser.add_argument(
        '--test-feeder-args',
        action=DictAction,
        default=dict(),
    ...
    parser.add_argument(
        '--eval-batch-size', type=int, default=1, help='eval batch size')
    parser.add_argument(
    ...
    parser.add_argument('--loss-type', type=str, default='BlvLoss')
    parser.add_argument('--cls-num-list', type=int, default=[0], nargs='+')
    parser.add_argument('--clip-grad-norm', type=bool, default=False)
    parser.add_argument('--extract',type=str2bool,
                        default=False,
...
class Processor():
    """ 
        Processor for Skeleton-based Emotion Recgnition
    """
    ...
    def load_data(self):
        ...
        if self.arg.phase == 'train':
            ...
            self.data_loader['eval'] = torch.utils.data.DataLoader(
                ...
                batch_size=self.arg.eval_batch_size,
                shuffle=False,
                num_workers=self.arg.num_worker,
                drop_last=False,
                worker_init_fn=init_seed)
    ...
    def load_model(self):
        ...
        elif self.arg.loss_type == 'BlvLoss':
            self.loss = BlvLoss(cls_num_list=self.arg.cls_num_list).cuda(output_device)
        else:
    ...
    def save(self, state, is_best):
        ...
        if is_best:
            checkpoint_path = os.path.join(
                self.arg.work_dir, "model_best.pt")
            torch.save(state, checkpoint_path)

        # save the latest model for resume
        checkpoint_path = os.path.join(
            self.arg.work_dir, "model_latest.pt")
        torch.save(state, checkpoint_path)
    ...
    def eval(self, epoch, save_score=False, loader_name=['test'], wrong_file=None, result_file=None, extract=False):
        ...
        self.model.eval()
        self.print_log('{} epoch: {}'.format(loader_name[0], epoch + 1))
        for ln in loader_name:
            ...
            if ln == 'test':
                ...
                self.print_log('test weighted F1: {:.4f}%'.format(F1 * 100))

            self.print_log('Mean {} acc: {:.4f}%'.format(ln, accuracy*100))

            if ln == 'eval':
                is_best = True if accuracy > self.best_acc else False
                if is_best:
                ...
                self.save({'epoch': epoch,
                        ...
                        'state_dict': weights,
                        'acc': accuracy,
                        'optimizer': self.optimizer.state_dict()}, is_best=is_best)
    ...
    def start(self):
        if self.arg.phase == 'train':
            ...
            self.eval(epoch=0, save_score=self.arg.save_score, loader_name=['test'], wrong_file=wf, result_file=rf, extract=arg.extract)
            self.print_log('Done.\n')

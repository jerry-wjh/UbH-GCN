...
class Feeder(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.bone:
            from .bone_pairs import aide_pairs_upper_head
            bone_data_numpy = np.zeros_like(data_numpy) # 3, T, V
            for v1, v2 in aide_pairs_upper_head:
                bone_data_numpy[:, :, v1 - 1] = data_numpy[:, :, v1 - 1] - data_numpy[:, :, v2 - 1]
            data_numpy = bone_data_numpy
        if self.angle:
            from .bone_pairs import aide_bone_angle_pairs_upper_head
            ...
            for a_key in aide_bone_angle_pairs_upper_head.keys():
                # bone angles
                v1, v2 = aide_bone_angle_pairs_upper_head[a_key]
